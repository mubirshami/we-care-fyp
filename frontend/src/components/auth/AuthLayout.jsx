...
export function PasswordStrength({ password }) {
  ...
  const [barColor, labelColor, label] =
    score <= 1 ? ['bg-error-500',   'text-error-600',   'Weak'] :
    score <= 2 ? ['bg-warning-500', 'text-warning-600', 'Fair'] :
    score <= 3 ? ['bg-primary-400', 'text-primary-600', 'Good'] :
...
export function PasswordInput({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {error  && <p id={errorId} role="alert" className="text-xs text-error-600">{error}</p>}
      {!error && hint && <p id={hintId} className="text-xs text-neutral-500">{hint}</p>}
    </div>
  );
}
        ...
        <div className="space-y-8">
          <div>
            <h1 className="text-4xl xl:text-5xl font-display font-semibold text-white leading-tight tracking-tight">
              A calmer mind<br />starts here.
            </h1>
            <p className="mt-4 text-base leading-relaxed max-w-sm" style={{ color: 'rgba(255,255,255,0.65)' }}>
              Reflect on your emotions, build self-awareness, and take steps toward better mental wellbeing — at your own pace.
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-neutral-50">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-10 lg:px-12 lg:overflow-y-auto">
        {/* Mobile brand mark — hidden on lg (brand panel takes over) */}
