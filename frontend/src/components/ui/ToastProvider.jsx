...
function Toast({ id, message, type, onDismiss }) {
  ...
  return (
    ...
    >
      {/* Left color bar */}
      <span
        className={`absolute inset-y-0 left-0 w-1 rounded-l-card ${cfg.bar}`}
        aria-hidden="true"
      />

      {/* Icon */}
      <span
        className={`ml-2 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${cfg.icon}`}
      >
