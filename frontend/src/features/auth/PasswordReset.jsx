  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <Link to="/forgotpassword" className={['inline-flex h-11 w-full items-center justify-center', 'rounded-button bg-primary-500 text-sm font-semibold text-white', 'hover:bg-primary-600 transition-all duration-150 shadow-primary'].join(' ')}>
          Request a new link
        </Link>
...
function ResetForm({ id, token }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Something went wrong. Please try requesting a new reset link.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-neutral-500">Choose a strong password to protect your account.</p>
      </div>
      <form className="space-y-5" onSubmit={handleSubmit} noValidate>
        <PasswordInput id="reset-password" label="New password" value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} hint="At least 8 characters" autoComplete="new-password" showStrength />
        <PasswordInput id="reset-confirm" label="Confirm new password" value={confirm} onChange={(e) => { setConfirm(e.target.value); setError(''); }} error={confirmError} autoComplete="new-password" placeholder="Re-enter your password" />
        {error && (
        ...
        <button type="submit" disabled={!canSubmit} className={['w-full h-11 inline-flex items-center justify-center gap-2', 'rounded-button bg-primary-500 text-sm font-semibold text-white', 'hover:bg-primary-600 active:bg-primary-700', 'disabled:opacity-60 disabled:cursor-not-allowed', 'transition-all duration-150 shadow-primary'].join(' ')}>
          {loading ? (<><AuthSpinner /><span>Updating password…</span></>) : 'Update password'}
        </button>
      </form>
    </div>
  );
