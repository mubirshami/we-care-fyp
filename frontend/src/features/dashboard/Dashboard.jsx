...
const INSIGHT_CFG = {
  positive: {
    ...
    message:
      "Your recent entries reflect a positive outlook. Something's working — keep noticing it.",
  ...
  negative: {
    ...
    message:
      "Your entries carry some heaviness lately. That's okay — writing it down is already a form of care.",
  ...
  neutral: {
    ...
    message:
      'Your recent entries show a calm, even state of mind. Quiet consistency has its own kind of strength.',
...
function TodayCardSkeleton() {
  return (
    <div
      className="rounded-card border border-neutral-200 bg-white p-6 shadow-card space-y-4"
      role="status"
      aria-label="Loading"
    >
...
function PromptCard({ prompt }) {
  return (
    ...
    >
      ...
      <h2 className="text-lg font-display font-semibold text-neutral-900 leading-snug mb-1">
        {prompt}
      </h2>
    ...
    >
      ...
      <div className="flex gap-3">
        <Link
          ...
          className={[
            'flex h-9 flex-1 items-center justify-center rounded-button',
            'border border-success-300 bg-white text-xs font-semibold text-success-700',
            'hover:bg-success-50 transition-all duration-150',
          ].join(' ')}
        >
          View entry
        </Link>
        <Link
          ...
          className={[
            'flex h-9 flex-1 items-center justify-center rounded-button',
            'bg-success-500 text-xs font-semibold text-white',
            'hover:bg-success-600 transition-all duration-150',
          ].join(' ')}
        >
          Write another
        </Link>
  ...
  return (
    <div className="rounded-card border border-neutral-200 bg-white p-4 shadow-card">
      ...
      <div
        ...
        role="list"
        aria-label="Weekly journal activity"
      >
        {weekDots.map((dot, i) => (
              ...
              className={[
                ...
                dot.hasEntry
                  ...
                  : dot.isFuture
                    ? 'bg-neutral-100'
                    : 'bg-neutral-200',
                dot.isToday ? 'ring-2 ring-primary-300 ring-offset-1' : '',
              ].join(' ')}
            />
            <span
              className={`text-[10px] font-medium leading-none ${dot.isToday ? 'text-primary-600' : 'text-neutral-400'}`}
            >
              {dot.label}
            </span>
          </div>
...
function ErrorBanner({ onRetry }) {
  return (
    <div
      role="alert"
      className="flex items-start gap-3 rounded-card border border-error-100 bg-error-50 px-4 py-3"
    >
      <svg
...
export default function Home() {
  ...
  const recentEntries = journals.filter((j) => !todayEntry || j._id !== todayEntry._id).slice(0, 2);
        ...
        <section aria-label="Recent reflections">
          ...
          <ul className="space-y-3">
            {recentEntries.map((journal) => (
              <li key={journal._id}>
                <ReflectionCard journal={journal} />
              </li>
            ))}
          </ul>
        </section>
