...
import { getEntryDate, firstSentence, formatRelative, formatTime } from '../../utils/journal';
...
// ── Completion insight config (Journal-specific) ──────────────────────────────
...
function getInsightCfg(sentiment) {
  return INSIGHT_CFG[(sentiment || '').toLowerCase()] || INSIGHT_CFG.neutral;
}
...
function CompletionView({ result, onWriteAnother }) {
  const insight = getInsightCfg(result.sentiment);
  const preview = firstSentence(result.content, 140);
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className={`mt-1 text-sm leading-relaxed ${insight.subtextCls}`}>
              {insight.subtext}
            </p>
          </div>
        </div>
...
function WriteTab({ token }) {
  ...
  return (
    <div className="space-y-5">
      {/* Daily prompt */}
      <div
        className="rounded-card border border-primary-100 px-5 py-4"
        style={{ backgroundColor: 'var(--color-primary-50)' }}
      >
        <p className="text-xs font-semibold uppercase tracking-wider text-primary-500 mb-1.5">
          Reflection prompt
        </p>
        <p
          className="text-sm font-medium leading-relaxed"
          style={{ color: 'var(--color-primary-900, #1e1b4b)' }}
        >
          {prompt}
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 px-5 pt-4 pb-3 border-b border-neutral-100">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            <time className="text-xs font-semibold text-neutral-700" dateTime={date.toISOString()}>
              {formatRelative(date)}
            </time>
            <span className="text-neutral-300 text-xs select-none" aria-hidden="true">
              ·
            </span>
    ...
    <>
      ...
      <ul className="space-y-3" aria-label="Journal entries">
        {journals.map((journal) => (
          <li key={journal._id}>
            <EntryCard journal={journal} onEdit={setEditingJournal} onDelete={setDeleteId} />
          </li>
        ))}
      </ul>
