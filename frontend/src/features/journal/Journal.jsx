      ...
      <div className="rounded-card border border-neutral-200 overflow-hidden">
        <Editor
          ...
          init={{
            ...
            content_style: "body { font-family: 'Inter', system-ui, sans-serif; font-size: 15px; line-height: 1.7; color: #292524; padding: 16px; }",
            skin: 'oxide',
          }}
        />
      </div>
...
function HistoryTab({ token }) {
  ...
  if (!journals.length) {
    return (
      <div className="flex flex-col items-center py-16 text-center">
        ...
        <p className="mt-1 text-sm text-neutral-500">Your saved journal entries will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 mb-2">
                    <time className="text-xs font-medium text-neutral-500" dateTime={date.toISOString()}>{formatDate(date)}</time>
                ...
                <Button variant="ghost" size="sm" onClick={() => setDeleteId(journal._id)} aria-label="Delete entry" className="shrink-0 text-neutral-400 hover:text-error-600">
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete entry" description="This entry will be permanently removed and cannot be recovered." size="sm">
...
export default function Journal() {
  ...
  return (
    <div className="max-w-[780px] mx-auto px-4 py-6 sm:px-6 sm:py-8">
      ...
      <p className="text-sm text-neutral-500 mb-6">Write freely, track your emotions, and revisit past entries.</p>
      <div className="flex gap-1 border-b border-neutral-200 mb-6" role="tablist" aria-label="Journal sections">
        <Tab active={activeTab === 'write'} onClick={() => setTab('write')}>✍️ Write</Tab>
        <Tab active={activeTab === 'history'} onClick={() => setTab('history')}>📖 History</Tab>
      </div>
      <div role="tabpanel">
