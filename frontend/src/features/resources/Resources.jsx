  ...
  return (
    <section className="space-y-4">
      <h2 className="text-base font-semibold text-neutral-800 flex items-center gap-2">
        ...
        {title}
        <span className="ml-1 rounded-pill bg-neutral-100 px-2 py-0.5 text-xs font-medium text-neutral-500">
          {videos.length}
        </span>
      </h2>
      <div className="grid gap-5 sm:grid-cols-2">
        {videos.map((vid) => (
          <div
            key={vid._id}
            className="rounded-card overflow-hidden border border-neutral-200 bg-white shadow-card"
          >
    ...
    return (
      <div className="flex flex-col items-center py-16 text-center">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Check back soon — wellness videos will be added here.
        </p>
      </div>
    );
  }
  ...
  if (!books.length) {
    return (
      <div className="flex flex-col items-center py-16 text-center">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Wellness reading recommendations will appear here.
        </p>
      </div>
    );
  }
      ...
      {books.map((book) => (
        <li key={book._id}>
          ...
          >
            ...
            <svg
              className="h-4 w-4 shrink-0 text-neutral-400 group-hover:text-primary-500 transition-colors"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={2}
              stroke="currentColor"
  ...
  return (
    <div className="max-w-[900px] mx-auto px-4 py-6 sm:px-6 sm:py-8">
      ...
      <p className="text-sm text-neutral-500 mb-6">
        Guided exercises, music for focus, and recommended reading for your wellness journey.
      </p>
      <div
      ...
      >
        <Tab active={activeTab === 'videos'} onClick={() => setTab('videos')}>
          🎬 Videos
        </Tab>
        <Tab active={activeTab === 'books'} onClick={() => setTab('books')}>
          📚 Books
        </Tab>
      </div>
      <div role="tabpanel">
