...
// Extract first readable sentence up to maxLen chars
...
function calcStreak(journals) {
  ...
  const uniqueDays = [
    ...new Set(journals.map((j) => getEntryDate(j).toDateString())),
...
// State A — user has not journaled today
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-neutral-500 mb-6">
        Write freely — there are no rules here.
      </p>
...
// State B — user has already journaled today
...
function TodayCard({ todayEntry, prompt, loading }) {
  ...
  return todayEntry
    ? <CompletionCard todayEntry={todayEntry} />
          ...
          <div key={i} role="listitem" className="flex flex-col items-center gap-1.5">
            ...
            <span
              className={`text-[10px] font-medium leading-none ${
                dot.isToday ? 'text-primary-600' : 'text-neutral-400'
              }`}
            >
              {dot.label}
            </span>
...
export default function Home() {
  ...
  // Recent entries: exclude today's entry (shown in primary card) — show last 2
